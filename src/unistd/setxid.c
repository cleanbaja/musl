...
static void do_setxid(void *p)
{
	struct ctx *c = p;
	if (c->ret<0) return;
	int ret = __syscall(c->nr, c->type, c->id, c->eid, c->sid);
	if (ret && !c->ret) {
		...
		__block_all_sigs(0);
		__syscall(SYS_kill, __syscall(SYS_get_id, ID_PID), SIGKILL);
	}
	c->ret = ret;
}

int __setxid(int nr, int type, int id, int eid, int sid)
{
	/* ret is initially nonzero so that failure of the first thread does not
	 * trigger the safety kill above. */
	struct ctx c = { .nr = nr, .type = type, .id = id, .eid = eid, .sid = sid, .ret = 1 };
